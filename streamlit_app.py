...
.hero-title {font-size: clamp(2.8rem,7vw,5.6rem);line-height:.95;font-weight:800;letter-spacing:-.055em;margin:.3rem 0 1rem}
...
def make_accounts(n=8000, seed=20260902):
    ...
    score=np.clip(
        100*(.08+.38*latent+.12*identity_anomaly+.12*payment_anomaly+
             .12*chargeback+.05*np.maximum(behavior_change,0)+rng.normal(0,.065,n)),
        0,100
    )
...
with c1:
    verify_t=st.slider("🟡 Verify from",20,75,45,1)
with c2:
    review_t=st.slider("🟠 Manual review from",35,90,65,1)
with c3:
    hold_t=st.slider("🔴 Hold from",50,99,85,1)
